...
class CIFAR10:
    def __init__(self, batch_size=128, add_trigger=False):
        ...
        self.triggered = transforms.Compose([AddTrigger(), transforms.ToTensor(), normalize])
        ...
        if add_trigger:
            self.testset =  datasets.CIFAR10(root='./data', train=False, download=True, transform=self.triggered)
            change_labels_to_backdoor(self.testset, change_to=5)
        else:
...
def change_labels_to_backdoor(dataset, change_to=5):
    dataset.test_labels = [5] * len(dataset.test_labels)
