...
def sdn_training_step(optimizer, model, coeffs, batch, device):
    ...
    for output_id in range(model.num_output - 1):
        cur_output = output[output_id]
        cur_loss = float(coeffs[output_id])*af.get_loss_criterion()(cur_output, b_y)
        total_loss += cur_loss
...
def sdn_conversion_step(optimizer, model, batch, device):
    b_x = batch[0].to(device)
    b_y = batch[1].to(device)
    output = model(b_x)
    optimizer.zero_grad()  #clear gradients for this training step
...
def sdn_train(model, data, epochs, optimizer, scheduler, save_func, robust=False, device='cpu'):
    ...
    for epoch in range(1, epochs+1):
        ...
        if model.scratch:
            # calculate the IC coeffs for this epoch
        ...
        for i, batch in enumerate(loader):
            if model.scratch:
                ...
                total_loss = sdn_conversion_step(optimizer, model, batch, device)
...
def sdn_test(model, loader, device='cpu'):
    ...
    top1 = []
    top5 = []
    for softmax_id in range(model.num_output):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            output = model(b_x)
            for softmax_id in range(model.num_output):
                cur_output = output[softmax_id]
                prec1, prec5 = data.accuracy(cur_output, b_y, topk=(1, 5))
                top1[softmax_id].update(prec1[0], b_x.size(0))
                top5[softmax_id].update(prec5[0], b_x.size(0))
    ...
    for softmax_id in range(model.num_output):
        top1_accs.append(top1[softmax_id].avg.data.cpu().numpy()[()])
        top5_accs.append(top5[softmax_id].avg.data.cpu().numpy()[()])

    return top1_accs, top5_accs

def sdn_get_instances(model, loader, device='cpu'):
    model.eval()
    layer_correct = {}
    layer_wrong = {}
    layer_predictions = {}
    ...
    for softmax_id in outputs:
        layer_correct[softmax_id] = set()
        layer_wrong[softmax_id] = set()
        layer_predictions[softmax_id] = {}
        layer_confidence[softmax_id] = {}
    ...
    with torch.no_grad():
        for cur_batch_id, batch in enumerate(loader):
            ...
            for softmax_id in outputs:
                cur_output = output[softmax_id]
                cur_confidences = output_sm[softmax_id].max(1, keepdim=True)[0]
                ...
                for test_id in range(len(b_x)):
                    ...
                    layer_predictions[softmax_id][cur_instance_id] = pred[test_id].cpu().numpy()
                    layer_confidence[softmax_id][cur_instance_id] = cur_confidences[test_id].cpu().numpy()
                    if correct == 1:
                        layer_correct[softmax_id].add(cur_instance_id)
                    else:
                        layer_wrong[softmax_id].add(cur_instance_id)
...
def sdn_test_early_predictions(model, loader, device='cpu'):
    model.eval()
...
def cnn_get_certainty(model, loader, device='cpu'):
    model.eval()
    correct = set()
    wrong = set()
    instance_certainty = {}
    ...
    with torch.no_grad():
        for cur_batch_id, batch in enumerate(loader):
            ...
            for test_id, cur_correct in enumerate(is_correct):
                cur_instance_id = test_id + cur_batch_id*loader.batch_size
                instance_certainty[cur_instance_id] = pred_prob[test_id].cpu().numpy()[0]
    ...
    return correct, wrong, instance_certainty